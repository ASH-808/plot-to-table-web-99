...
const FeaturedPlots = () => {
  ...
  const filteredPlots = availablePlots.filter(plot => {
    ...
    const matchesSearch = plot.type.toLowerCase().includes(searchTerm.toLowerCase()) || plot.location.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-md mx-auto mb-8">
          <div className="relative">
            ...
            <input type="text" placeholder="Search by crop type or area..." className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-full focus:ring-2 focus:ring-green-500 focus:border-transparent" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        </div>

        {/* Filter Tabs */}
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {filters.map(filter => <button key={filter.id} onClick={() => setSelectedFilter(filter.id)} className={`px-6 py-3 rounded-full font-semibold transition-all duration-200 flex items-center gap-2 transform hover:scale-105 ${selectedFilter === filter.id ? 'bg-green-600 text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {filteredPlots.map((plot, index) => <Card key={plot.id} className="group card-hover overflow-hidden border-0 shadow-lg bg-white animate-fade-in" style={{
          animationDelay: `${index * 0.1}s`
        }}>
              <div className="relative overflow-hidden">
                <img src={plot.image} alt={`${plot.type} in ${plot.location}`} className="w-full h-64 object-cover transition-transform duration-300 group-hover:scale-110" />
              ...
              <CardContent className="p-6">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {plot.features.map((feature, idx) => <span key={idx} className="bg-green-100 text-green-700 px-2 py-1 rounded text-xs">
                        {feature}
                      </span>)}
                  </div>
                </div>
