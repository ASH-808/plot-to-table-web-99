  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-green-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            {navItems.map((item, index) => (
              <button
                ...
                className="text-gray-700 hover:text-green-600 font-medium transition-all duration-300 relative group animate-fade-in text-sm xl:text-base"
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="lg:hidden absolute top-16 sm:top-18 left-0 right-0 bg-white border-b border-green-100 shadow-lg animate-slide-up">
            <div className="px-4 py-6 space-y-4">
              {navItems.map((item) => (
                <button
                  key={item.label}
                  onClick={() => scrollToSection(item.href)}
                  className="block w-full text-left text-gray-700 hover:text-green-600 font-medium py-3 transition-colors duration-200 border-b border-gray-100 last:border-b-0"
                >
                  {item.label}
                </button>
              ))}
